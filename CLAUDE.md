...
3. Lees daarna de gevonden bestanden voor de volledige context. `zoek` toont het volledige pad. Het archief staat meestal in Google Drive (omgevingsvariabele `MAINFRAME_ARCHIEF`), niet in de map `archief/` van de repository.
...
- Het archief (de map in `MAINFRAME_ARCHIEF`, en `archief/`, `inbox/` en `.mainframe/`) is privé. Commit of push het nooit en stuur het niet naar externe diensten.
